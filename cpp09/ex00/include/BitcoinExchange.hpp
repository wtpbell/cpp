...
class BitcoinExchange
{
	public:
		...
		BitcoinExchange& operator=(BitcoinExchange&& other) noexcept = default;

		bool					loadData(const std::filesystem::path& filePath);
		void					logData(void);
		static bool				validateDate(const std::chrono::year_month_day& date);
		template <typename T>
		constexpr static bool	validateValue(T value);

	private:
		const std::chrono::year_month_day minDate_ = std::chrono::year_month_day(std::chrono::year{ 2009 }, std::chrono::month{ 1 }, std::chrono::day{ 2 });
		const std::chrono::year_month_day maxDate_ = std::chrono::floor<std::chrono::days>(std::chrono::system_clock::now());
		std::map<std::chrono::year_month_day, float> exchangeData_;
};
